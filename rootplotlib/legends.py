...
def add_text(x,y,text,color=1, textfont=42, textsize=0.1, pad=None, fig=None):
    if fig is None:
        fig = rpl.get_figure()
    pad = fig.get_pad(pad)
    pad.cd()
...
def add_legend(legends, x1=.8, y1=.8, x2=.9, y2=.9,
               pad=None, textsize=18, ncolumns=1,
               option='f', squarebox=True, title='',
               fig=None):
